...
class FakeCollection {
  ...
  update(query, data, callback) {
    ...
    this.findOne(query, (error, record) => {
      ...
      var dataToUpdate = copyObject(record);
  ...
  find(query, callback) {
    ...
    try {
      result = this.data.find(formatForQuery(query));
  ...
  findOne(query, callback) {
    ...
    try {
      if (this._isObjectId(query)) {
        result = this.data.find(formatForQuery({_id: query}));
      } else {
        result = this.data.find(formatForQuery(query));
      }
    } catch (error) {
    ...
    callback(null, formatForResponse(result[0]) || null);
  }
